...
func SetupRouter(
	...
	// 初始化API控制器
	userController := initControllers(
		userService)
	// 配置HTTP路由
	setupHTTPRoutes(r, userController, cfg) //用户
...
2. 定义用户相关路由
*/
func setupHTTPRoutes(r *gin.Engine, userController *http_controller.UserController, cfg *config.AppConfig) {
	// 从配置加载HTTP路由
	...
	for _, group := range cfg.HTTPRoutes {
		...
		for _, route := range group.Routes {
			...
			handler := getHTTPHandler(userController, route.Handler)
			if handler == nil {
...
// getHTTPHandler 根据名称获取对应的处理函数
func getHTTPHandler(inController *http_controller.UserController, name string) gin.HandlerFunc {
	switch name {
	case "UserRegister":
